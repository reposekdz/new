...
import { Terminal as TerminalIcon, Trash2, Maximize2, Minimize2, Wrench, ArrowDown, Sparkles } from 'lucide-react';
...
const Row = memo(({ index, style, data }: { index: number, style: React.CSSProperties, data: any }) => {
    const { logs, isRunning, input, setInput, handleKeyDown, inputRef, onAutoFix } = data;

    if (index === logs.length) {
        // Input Row
        return (
            <div style={style} className="flex items-center gap-2 px-3 bg-[#0c0c0c]">
    ...
    return (
        <div style={style} className={`px-3 py-1 break-all whitespace-pre-wrap leading-snug group/line text-sm hover:bg-white/5 transition-colors ${log.type === 'command' ? 'font-bold mt-2 mb-1' : ''}`}>
            {log.type === 'command' ? (
                ...
                <div className="flex items-start gap-2 relative">
                    ...
                    {log.type === 'stderr' && onAutoFix && (
                        <button 
                            ...
                            className="absolute right-0 top-0 opacity-0 group-hover/line:opacity-100 flex items-center gap-1.5 px-2 py-1 bg-red-950/80 border border-red-500/30 rounded text-[10px] text-red-200 hover:bg-red-900 hover:text-white hover:border-red-400 transition-all backdrop-blur-md shadow-lg shadow-red-900/20 translate-x-2 group-hover/line:translate-x-0"
                            title="Ask OmniGen to fix this error"
                        >
    ...
    // Only re-render row if it's the input row or if the log at this index changed (unlikely)
    const isInputRow = prev.index === prev.data.logs.length;
    if (isInputRow) return prev.data.input === next.data.input && prev.data.isRunning === next.data.isRunning;
    return prev.data.logs[prev.index] === next.data.logs[next.index];
});
